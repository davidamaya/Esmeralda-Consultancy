...
Files (all 8 go at the ROOT of the repo, not inside a folder):
...
Deploy:
  ...
  2. Upload all 8 files to the repo root, commit.
  3. Settings -> Pages -> Deploy from a branch -> main / (root) -> Save.
  4. Live in ~2 min at davidamaya.github.io/Esmeralda-Consultancy/

Updating an existing repo:
  Upload the 8 files themselves, not the folder. Delete any leftover
...
Photo reading: the review form reads notice photos on the visitor's device
(tesseract.js from jsdelivr CDN) and attaches the photo to the FormSubmit
email (10MB limit). First real submission triggers FormSubmit's one-time
activation email to your address.
