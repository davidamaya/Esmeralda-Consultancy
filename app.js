...
function goNotice(e){
  ...
  var i = document.getElementById('nsearch');
  var q = i && i.value ? i.value.trim() : '';
  location.href = withLang('notices.html' + (q ? '?q=' + encodeURIComponent(q) : ''), LANG);
  return false;
}
...
function setLang(l){
  ...
  if (ns) ns.placeholder = l === 'es' ? 'Pruebe 1444, 1080CZ, sobrepago, audiencia\u2026' : 'Try 1444, 1080CZ, overpayment, hearing\u2026';
...
var TSTR = {
  en:{ clockL:'Your clock', pathH:'Your path', gatherH:'Gather now', noteH:'Our take', change:'Change', dateL:'Date printed on your notice', pf1:'My situation: ', pfDate:' The date printed on my notice is ', pfWhat:'\n\nWhat happened: ',
  ...
  es:{ clockL:'Su reloj', pathH:'Su camino', gatherH:'Junte desde ahora', noteH:'Nuestra opini\u00f3n', change:'Cambiar', dateL:'Fecha impresa en su aviso', pf1:'Mi situaci\u00f3n: ', pfDate:' La fecha impresa en mi aviso es ', pfWhat:'\n\nLo que pas\u00f3: ',
...
function renderTriage(){
  ...
  html += '<div class="tri-facts"><div class="tf"><span class="tfl">' + t.clockL + '</span><span class="tfv">' + s.clock[LANG] + '</span></div></div>';
  if (s.win){
    html += '<div class="tri-when"><label class="field" for="triD">' + t.dateL + '</label>' +
            ...
            '<div class="dl-out" id="dlOut"><div class="small" style="text-transform:uppercase;letter-spacing:.1em;font-weight:700">' + t.fileBy + '</div>' +
