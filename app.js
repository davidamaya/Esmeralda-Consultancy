...
function toggleMenu(b){
  ...
  var n = (id && document.getElementById(id)) || document.querySelector('nav.main');
  if (!n) return;
...
function setLang(l){
  ...
  if (be){ be.classList.toggle('on', l === 'en'); be.setAttribute('aria-pressed', String(l === 'en')); }
  if (bs){ bs.classList.toggle('on', l === 'es'); bs.setAttribute('aria-pressed', String(l === 'es')); }
...
/* ================= California deadline calendar =================
   Practical deadline calculator: rolls weekends and observed California
   state-office holidays published by CalHR. It is still an estimate; the
   notice, agency rules, and filing method control. */
function dateKey(d){
  return d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0') + '-' + String(d.getDate()).padStart(2, '0');
}
function nthWeekday(year, month, weekday, n){
  var d = new Date(year, month, 1);
  var add = (weekday - d.getDay() + 7) % 7;
  d.setDate(1 + add + (n - 1) * 7);
  return d;
}
function lastWeekday(year, month, weekday){
  var d = new Date(year, month + 1, 0);
  var sub = (d.getDay() - weekday + 7) % 7;
  d.setDate(d.getDate() - sub);
  return d;
}
function addHoliday(map, d, en, es, opts){
  opts = opts || {};
  var observed = new Date(d.getFullYear(), d.getMonth(), d.getDate());
  if (observed.getDay() === 0){ observed.setDate(observed.getDate() + 1); }
  /* CalHR observes Sunday holidays on Monday. Saturday holidays are already non-business days. */
  if (observed.getDay() !== 6){ map[dateKey(observed)] = {en:en, es:es}; }
}
function caObservedHolidayMap(year){
  var map = {};
  addHoliday(map, new Date(year, 0, 1), 'New Year\u2019s Day', 'A\u00f1o Nuevo');
  addHoliday(map, nthWeekday(year, 0, 1, 3), 'Martin Luther King Jr. Day', 'D\u00eda de Martin Luther King Jr.');
  addHoliday(map, nthWeekday(year, 1, 1, 3), 'Presidents\u2019 Day', 'D\u00eda de los Presidentes');
  addHoliday(map, new Date(year, 2, 31), 'Cesar Chavez Day', 'D\u00eda de C\u00e9sar Ch\u00e1vez');
  addHoliday(map, lastWeekday(year, 4, 1), 'Memorial Day', 'D\u00eda de los Ca\u00eddos');
  addHoliday(map, new Date(year, 6, 4), 'Independence Day', 'D\u00eda de la Independencia');
  addHoliday(map, nthWeekday(year, 8, 1, 1), 'Labor Day', 'D\u00eda del Trabajo');
  addHoliday(map, new Date(year, 10, 11), 'Veterans Day', 'D\u00eda de los Veteranos');
  var thanksgiving = nthWeekday(year, 10, 4, 4);
  addHoliday(map, thanksgiving, 'Thanksgiving Day', 'D\u00eda de Acci\u00f3n de Gracias');
  addHoliday(map, new Date(year, 10, thanksgiving.getDate() + 1), 'Day after Thanksgiving', 'D\u00eda despu\u00e9s de Acci\u00f3n de Gracias');
  addHoliday(map, new Date(year, 11, 25), 'Christmas Day', 'Navidad');
  return map;
}
function caObservedHoliday(d){
  var y = d.getFullYear();
  var k = dateKey(d);
  return caObservedHolidayMap(y)[k] || caObservedHolidayMap(y - 1)[k] || caObservedHolidayMap(y + 1)[k] || null;
}
function rollToBusinessDay(d){
  var rolledWeekend = false, rolledHoliday = false, holidays = [];
  var guard = 0;
  while (guard++ < 14){
    var h = caObservedHoliday(d);
    if (d.getDay() === 0 || d.getDay() === 6){ rolledWeekend = true; d.setDate(d.getDate() + 1); continue; }
    if (h){ rolledHoliday = true; holidays.push(h); d.setDate(d.getDate() + 1); continue; }
    break;
  }
  return {date:d, weekend:rolledWeekend, holiday:rolledHoliday, holidays:holidays};
}
...
var SCEN = {
  worker: [
    ...
    { id:'w1',
      ...
      forum:'EDD · CUIAB',
      title:{en:'A misconduct denial turns on proof, not labels.', es:'Una negación por mala conducta depende de la prueba, no de la etiqueta.'},
    ...
    { id:'w4',
      ...
      win:{days:30},
      path:{en:['Appeal within the window on your notice.','Get your medical certification tightened up. That is often where the case turns.','Hearing before a CUIAB judge, usually by phone.','Decision by mail, with 30 more days to the Appeals Board if needed.'],
...
var TSTR = {
  en:{ pathH:'Your path', gatherH:'Gather now', noteH:'What to know', change:'Change', dateL:'Date printed on your notice', pf1:'My situation: ', pfDate:' The date printed on my notice is ', pfWhat:'\n\nWhat happened: ', 
       fileBy:'File by', rolledWeekend:'moved past the weekend', rolledHoliday:'moved past a California state-office holiday', rolledBoth:'moved past the weekend and a California state-office holiday', today:'Due today. File it now.',
       ...
       estimateNote:'Estimate only. This tool rolls weekends and observed California state-office holidays. Agency rules, filing method, and the date printed on your own notice control.',
       holidayPrefix:'Holiday checked', cta:'Get help' },
  es:{ pathH:'Su camino', gatherH:'Junte desde ahora', noteH:'Qu\u00e9 saber', change:'Cambiar', dateL:'Fecha impresa en su aviso', pf1:'Mi situaci\u00f3n: ', pfDate:' La fecha impresa en mi aviso es ', pfWhat:'\n\nLo que pas\u00f3: ', 
       fileBy:'Presente a más tardar el', rolledWeekend:'recorrida por el fin de semana', rolledHoliday:'recorrida por un feriado estatal observado en California', rolledBoth:'recorrida por el fin de semana y un feriado estatal observado en California', today:'Vence hoy. Preséntelo ya.',
       ...
       estimateNote:'Solo es una estimación. Esta herramienta recorre fines de semana y feriados estatales observados en California. Las reglas de la agencia, el método de presentación y la fecha impresa en su propio aviso son las que mandan.',
       holidayPrefix:'Feriado revisado', cta:'Obtenga ayuda' },
};
...
function renderTriage(){
  ...
  pw.setAttribute('aria-pressed', String(triSide === 'worker'));
  pe.setAttribute('aria-pressed', String(triSide === 'employer'));
...
function triCalc(){
  ...
  var rolled = rollToBusinessDay(d);
  d = rolled.date;
  var t = TSTR[LANG];
  var now = new Date();
  ...
  var rollText = '';
  if (rolled.weekend || rolled.holiday){
    rollText = rolled.weekend && rolled.holiday ? t.rolledBoth : (rolled.holiday ? t.rolledHoliday : t.rolledWeekend);
    rollText = ' (' + rollText + ')';
  }
  document.getElementById('dlDate').textContent = fmt.format(d) + rollText;
  var pill = document.getElementById('dlPill'), note = document.getElementById('dlNote');
  var hNote = '';
  if (rolled.holiday && rolled.holidays.length){
    var seen = [];
    rolled.holidays.forEach(function(h){
      var name = h[LANG];
      if (seen.indexOf(name) === -1) seen.push(name);
    });
    hNote = ' ' + t.holidayPrefix + ': ' + seen.join(', ') + '.';
  }
  out.classList.remove('ok','warn','past');
  if (diff < 0){ out.classList.add('past'); pill.textContent = t.past(-diff); note.textContent = t.pastNote + hNote; note.style.display = 'block'; }
  else if (diff === 0){ out.classList.add('warn'); pill.textContent = t.today; note.textContent = t.estimateNote + hNote; note.style.display = 'block'; }
  else { out.classList.add(diff <= 14 ? 'warn' : 'ok'); pill.textContent = t.left(diff); note.textContent = t.estimateNote + hNote; note.style.display = 'block'; }
...
function setSide(s){
  SIDE = s;
  var pw = document.getElementById('pWorker'), pe = document.getElementById('pEmployer');
  if (pw){ pw.classList.toggle('on', s === 'worker'); pw.setAttribute('aria-pressed', String(s === 'worker')); }
  if (pe){ pe.classList.toggle('on', s === 'employer'); pe.setAttribute('aria-pressed', String(s === 'employer')); }
...
function renderAsk(){
  ...
  for (var j = 0; j < hits.length; j++){
    var link = hits[j].id ? '<div class="ask-actions"><a class="btn sm il" href="' + withLang('notices.html#' + hits[j].id, LANG) + '">' + (LANG === 'es' ? 'Abrir aviso' : 'Open notice') + '</a></div>' : '';
    html += '<div class="ask-hit"><h4>' + hits[j].titleHTML + '</h4>' +
            (hits[j].pHTML ? '<p>' + hits[j].pHTML + '</p>' : '') +
            (hits[j].clockHTML ? '<p class="nclock">' + hits[j].clockHTML + '</p>' : '') + link + '</div>';
  }
  out.innerHTML = html;
}
...
(function(){
  ...
  setLang(p.get('lang') === 'es' ? 'es' : (saved === 'es' ? 'es' : 'en'));
})();
...
window.addEventListener('hashchange', openNoticeFromHash);
