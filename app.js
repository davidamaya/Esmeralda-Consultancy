...
var TSTR = {
  en:{ pathH:'Your path', gatherH:'Gather now', noteH:'What to know', change:'Change', dateL:'Date printed on your notice', pf1:'My situation: ', pfN:'My notice: ', pfDate:' The date printed on my notice is ', pfWhat:'\n\nWhat happened: ', rdWait:'Reading the photo\u2026', rdOk:'Found: ', rdDate:', dated ', rdNo:'Couldn\u2019t read it. Type what you see on the notice.', rdSend:'Send this to us', rdClear:'Clear', rdUnk:'We couldn\u2019t identify this notice.',
       ...
       cta:'Get help' },
  es:{ pathH:'Su camino', gatherH:'Junte desde ahora', noteH:'Qu\u00e9 saber', change:'Cambiar', dateL:'Fecha impresa en su aviso', pf1:'Mi situaci\u00f3n: ', pfN:'Mi aviso: ', pfDate:' La fecha impresa en mi aviso es ', pfWhat:'\n\nLo que pas\u00f3: ', rdWait:'Leyendo la foto\u2026', rdOk:'Encontrado: ', rdDate:', con fecha ', rdNo:'No se pudo leer. Escriba lo que dice el aviso.', rdSend:'Env\u00edenoslo', rdClear:'Borrar', rdUnk:'No pudimos identificar este aviso.',
...
function renderTriage(){
  ...
  if (s.win){
    html += '<div class="tri-when"><label class="field" for="triD">' + t.dateL + '</label>' +
...
/* ================= read the notice on-device, fill the form ================= */
function loadTess(cb){
...
function OCRLOG(say){
  return { logger: function(m){
    if (m && m.status === 'recognizing text' && m.progress) say(TSTR[LANG].rdWait + ' ' + Math.round(m.progress * 100) + '%');
  } };
}
function parseNotice(raw){
  var up = raw.toUpperCase();
  var code = '';
  var m = up.match(/\bDE[\s\-\.]?(\d{3,4})\s?([A-Z]{1,2})?\b/);
  if (m) code = 'DE ' + m[1] + (m[2] || '');
  else if (/NOTICE\s+OF\s+ASSESSMENT/.test(up)) code = 'Notice of Assessment';
  else if (/NOTICE\s+OF\s+HEARING|AVISO\s+DE\s+AUDIENCIA/.test(up)) code = 'Notice of Hearing';
  else if (/RIGHT\s+TO\s+SUE/.test(up)) code = /EEOC/.test(up) ? 'EEOC Notice of Right to Sue' : 'Right-to-Sue notice';
  var kw = up.search(/MAIL\s*DATE|DATE\s*MAILED|MAILED\s*ON|ISSUE\s*DATE|DATE\s*OF\s*MAILING|DATE\s*ISSUED|FECHA\s*DE\s*ENV/);
  var zone = kw > -1 ? up.slice(kw, kw + 140) : up;
  var dISO = '';
  var dm = zone.match(/(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{2,4})/) || up.match(/(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{2,4})/);
  var MN = ['JANUARY','FEBRUARY','MARCH','APRIL','MAY','JUNE','JULY','AUGUST','SEPTEMBER','OCTOBER','NOVEMBER','DECEMBER'];
  if (dm){
    var yy = +dm[3]; if (yy < 100) yy += 2000;
    dISO = yy + '-' + ('0' + dm[1]).slice(-2) + '-' + ('0' + dm[2]).slice(-2);
  } else {
    var dm2 = (kw > -1 ? zone : up).match(new RegExp('(' + MN.join('|') + ')\\s+(\\d{1,2}),?\\s+(\\d{4})'));
    if (dm2) dISO = dm2[3] + '-' + ('0' + (MN.indexOf(dm2[1]) + 1)).slice(-2) + '-' + ('0' + dm2[2]).slice(-2);
  }
  var dTxt = '';
  if (dISO){
    var pp = dISO.split('-');
    var dd = new Date(+pp[0], +pp[1] - 1, +pp[2]);
    if (!isNaN(dd.getTime())) dTxt = new Intl.DateTimeFormat(LANG === 'es' ? 'es-MX' : 'en-US', { year:'numeric', month:'long', day:'numeric' }).format(dd);
  }
  return { code: code, digits: m ? m[1] : '', dISO: dISO, dTxt: dTxt };
}
function sideOf(P){
  var m = { '1101':'employer', '1545':'employer', '428':'employer' };
  return m[P.digits] || (P.code === 'Notice of Assessment' ? 'employer' : '');
}
function applyRead(raw, say){
  var T = TSTR[LANG];
  var P = parseNotice(raw);
  if (!P.code && !P.dTxt){ say(T.rdNo); return; }
  say(T.rdOk + (P.code || '?') + (P.dTxt ? T.rdDate + P.dTxt : '') + '.');
  var box = document.getElementById('fMsg');
  if (box && !box.value) box.value = T.pfN + (P.code || '?') + '.' + (P.dTxt ? T.pfDate + P.dTxt + '.' : '') + T.pfWhat;
  var side = sideOf(P);
...
if (fFile){
  fFile.addEventListener('change', function(){
    ...
    if (!file) return;
    var st = document.getElementById('rdStat');
    function say(msg){ if (st){ st.style.display = 'block'; st.textContent = msg; } }
    say(TSTR[LANG].rdWait);
    loadTess(function(err){
      if (err || !window.Tesseract){ say(TSTR[LANG].rdNo); return; }
      window.Tesseract.recognize(file, 'eng', OCRLOG(say)).then(function(res){
        applyRead((res && res.data && res.data.text) || '', say);
      }).catch(function(){ say(TSTR[LANG].rdNo); });
    });
  });
}

/* ================= front-door reader ================= */
var READ_LAST = null, READ_FILE = null, LIB = null;
var WIN_DAYS = { '1080':30, '1444':30, '2517':30, '1000':30, '1101':10, '428':60 };
function winFor(P){
  if (P.digits && WIN_DAYS[P.digits] !== undefined) return WIN_DAYS[P.digits];
  if (P.code === 'Notice of Assessment') return 30;
  return null;
}
function agencyOf(P){
  if (P.code === 'Notice of Hearing') return 'CUIAB';
  if (P.code === 'EEOC Notice of Right to Sue') return 'EEOC';
  if (P.code && P.code.indexOf('Right-to-Sue') === 0) return 'CRD';
  return P.code ? 'EDD' : '';
}
function loadLib(cb){
  if (LIB) return cb();
  fetch('notices.html').then(function(r){ return r.text(); }).then(function(t){
    ...
    doc.querySelectorAll('.ncard').forEach(function(c){
      ...
      if (h4) LIB.push({
        key: (h4.textContent || '').toUpperCase().replace(/[^A-Z0-9]+/g, ' ').trim(),
...
function libFind(code){
  ...
  var k = code.toUpperCase().replace(/[^A-Z0-9]+/g, ' ').trim();
  for (var i = 0; i < LIB.length; i++){
    if (LIB[i].key.indexOf(k) > -1) return LIB[i];
  }
  return null;
}
function computeWindow(dISO, days){
  var T = TSTR[LANG];
  var pp = dISO.split('-');
  var start = new Date(+pp[0], +pp[1] - 1, +pp[2]);
  if (isNaN(start.getTime())) return null;
  var d = new Date(start); d.setDate(d.getDate() + days);
  var rolled = false;
  if (d.getDay() === 6){ d.setDate(d.getDate() + 2); rolled = true; }
  else if (d.getDay() === 0){ d.setDate(d.getDate() + 1); rolled = true; }
  var loc = LANG === 'es' ? 'es-MX' : 'en-US';
  var dateTxt = new Intl.DateTimeFormat(loc, { weekday:'long', year:'numeric', month:'long', day:'numeric' }).format(d) + (rolled ? T.rolled : '');
  var today = new Date(); today.setHours(0,0,0,0);
  var diff = Math.round((d - today) / 86400000);
  var cls = diff < 0 ? 'past' : (diff <= 14 ? 'warn' : 'ok');
  var pill = diff < 0 ? T.past(Math.abs(diff)) : (diff === 0 ? T.today : T.left(diff));
  return { dateTxt: dateTxt, cls: cls, pill: pill, past: diff < 0 };
}
function readDate(v){ if (READ_LAST){ READ_LAST.dISO = v; updateReadDl(); } }
function updateReadDl(){
  var box = document.getElementById('rdDl');
  if (!box || !READ_LAST) return;
  var days = winFor(READ_LAST);
  if (days === null || !READ_LAST.dISO){ box.className = 'dl-out'; box.innerHTML = ''; return; }
  var w = computeWindow(READ_LAST.dISO, days);
  if (!w){ box.className = 'dl-out'; box.innerHTML = ''; return; }
  var T = TSTR[LANG];
  box.className = 'dl-out show ' + w.cls;
  box.innerHTML = '<div class="small" style="font-weight:700">' + T.fileBy + '</div>' +
                  '<div class="dl-date">' + w.dateTxt + '</div>' +
                  '<div class="dl-pill">' + w.pill + '</div>' +
                  (w.past ? '<div class="dl-note">' + T.pastNote + '</div>' : '');
}
function renderReadCard(){
  var card = document.getElementById('readCard');
  if (!card || !READ_LAST) return;
  var T = TSTR[LANG];
  var P = READ_LAST;
  var lib = libFind(P.code);
  var ag = agencyOf(P);
  var html = '';
  if (ag) html += '<span class="forum">' + ag + '</span>';
  html += '<h3>' + (lib ? lib.titleHTML : (P.code ? P.code : T.rdUnk)) + '</h3>';
  if (lib && lib.pHTML) html += '<p class="rc-p">' + lib.pHTML + '</p>';
  if (lib && lib.clockHTML) html += '<p class="nclock">' + lib.clockHTML + '</p>';
  if (winFor(P) !== null){
    html += '<div class="tri-when"><label class="field" for="hrD">' + T.dateL + '</label>' +
            '<input type="date" id="hrD" value="' + (P.dISO || '') + '" oninput="readDate(this.value)" onchange="readDate(this.value)">' +
            '<div class="dl-out" id="rdDl"></div></div>';
  }
  html += '<div class="read-act"><button class="btn" type="button" onclick="readSend()">' + T.rdSend + '</button>' +
          '<button class="chg" type="button" onclick="readClear()">' + T.rdClear + '</button></div>';
  card.innerHTML = html;
  card.hidden = false;
  updateReadDl();
}
function readClear(){
  READ_LAST = null; READ_FILE = null;
  var card = document.getElementById('readCard');
  if (card){ card.hidden = true; card.innerHTML = ''; }
  var hf = document.getElementById('heroFile'); if (hf) hf.value = '';
  var st = document.getElementById('hrStat'); if (st) st.style.display = 'none';
}
function readSend(){
  var T = TSTR[LANG], P = READ_LAST;
  try{
    if (READ_FILE && window.DataTransfer){
      var dt = new DataTransfer(); dt.items.add(READ_FILE);
      var ff = document.getElementById('fFile'); if (ff) ff.files = dt.files;
    }
  }catch(e){}
  if (P){
    var box = document.getElementById('fMsg');
    if (box && !box.value && (P.code || P.dTxt)) box.value = T.pfN + (P.code || '?') + '.' + (P.dTxt ? T.pfDate + P.dTxt + '.' : '') + T.pfWhat;
    var side = sideOf(P); if (side) setSide(side);
    var st = document.getElementById('rdStat');
    if (st && (P.code || P.dTxt)){ st.style.display = 'block'; st.textContent = T.rdOk + (P.code || '?') + (P.dTxt ? T.rdDate + P.dTxt : '') + '.'; }
  }
...
if (heroFile){
  heroFile.addEventListener('change', function(){
    ...
    if (!file) return;
    READ_FILE = file;
    var st = document.getElementById('hrStat');
    function say(m){ if (st){ st.style.display = 'block'; st.textContent = m; } }
    say(TSTR[LANG].rdWait);
    loadTess(function(err){
      if (err || !window.Tesseract){ say(TSTR[LANG].rdNo); return; }
      window.Tesseract.recognize(file, 'eng', OCRLOG(say)).then(function(res){
        var P = parseNotice((res && res.data && res.data.text) || '');
        if (!P.code && !P.dTxt){ say(TSTR[LANG].rdNo); }
        else if (st){ st.style.display = 'none'; }
        READ_LAST = P;
        loadLib(renderReadCard);
      }).catch(function(){ say(TSTR[LANG].rdNo); });
    });
  });
}
