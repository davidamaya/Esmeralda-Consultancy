...
(function(){
  ...
  function select(id, push){
    ...
    if (push && history.replaceState) history.replaceState(null, '', location.pathname + location.search + '#' + id);
  }
  tabs.forEach(function(t, i){
    t.addEventListener('click', function(){ select(t.dataset.panel, true); });
    t.addEventListener('keydown', function(e){
      if (e.key === 'ArrowRight' || e.key === 'ArrowLeft'){
        e.preventDefault();
        var n = (i + (e.key === 'ArrowRight' ? 1 : tabs.length - 1)) % tabs.length;
        tabs[n].focus();
        select(tabs[n].dataset.panel, true);
      }
    });
  });
  var h = location.hash.replace('#','');
  select(h === 'crd' ? 'crd' : 'edd', false);
  window.addEventListener('hashchange', function(){
    var h2 = location.hash.replace('#','');
    if (h2 === 'edd' || h2 === 'crd') select(h2, false);
  });
})();
...
var SCEN = {
  worker: [
    { id:'w1',
      label:{en:'I was fired and EDD denied my unemployment', es:'Me despidieron y el EDD me negó el desempleo'},
      forum:'EDD · CUIAB',
      title:{en:'A misconduct denial — the most winnable case at CUIAB.', es:'Una negación por mala conducta: el caso más ganable en la CUIAB.'},
      path:{en:['Appeal in writing within the window on your notice','Keep certifying for benefits every two weeks — no exceptions','CUIAB hearing before a judge; we prepare you and appear with you','Decision arrives by mail; if it\u2019s wrong, 30 more days to the Appeals Board'],
      ...
      clock:{en:'30 days from the mailing date on the Notice of Determination (Unemp. Ins. Code §1328).', es:'30 días desde la fecha de envío del Aviso de Determinación (Código de Seguro de Desempleo, §1328).'},
      win:{days:30},
    ...
    { id:'w2',
      label:{en:'I quit my job and EDD denied me', es:'Renuncié a mi trabajo y el EDD me negó'},
      forum:'EDD · CUIAB',
      title:{en:'A voluntary-quit case turns on good cause.', es:'Un caso de renuncia se decide por la causa justificada.'},
      path:{en:['Appeal in writing within the window on your notice','Keep certifying every two weeks while it\u2019s pending','At the hearing, the question is why you left and what you tried first','Decision by mail; 30 more days to the Board if needed'],
            es:['Apele por escrito dentro del plazo de su aviso','Siga certificando cada dos semanas mientras está pendiente','En la audiencia, la pregunta es por qué se fue y qué intentó antes','Decisión por correo; 30 días más para la Junta si hace falta']},
      clock:{en:'30 days from the mailing date on the Notice of Determination (Unemp. Ins. Code §1328).', es:'30 días desde la fecha de envío del Aviso de Determinación (Código de Seguro de Desempleo, §1328).'},
      win:{days:30},
    ...
    { id:'w4',
      label:{en:'My SDI or Paid Family Leave claim was denied', es:'Me negaron la incapacidad (SDI) o el permiso familiar (PFL)'},
      forum:'EDD · CUIAB',
      ...
      path:{en:['Appeal within the window on your notice','Get your medical certification tightened up — that\u2019s where these are won','Hearing before a CUIAB judge, usually by phone','Decision by mail; 30 more days to the Board if needed'],
            es:['Apele dentro del plazo de su aviso','Refuerce su certificación médica: ahí se ganan estos casos','Audiencia ante un juez de la CUIAB, normalmente por teléfono','Decisión por correo; 30 días más para la Junta si hace falta']},
    ...
    { id:'w5',
      ...
      note:{en:'If the record from the first hearing is thin, be honest with yourself about the odds — and we\u2019ll be honest with you.', es:'Si el registro de la primera audiencia quedó flojo, sea honesto consigo mismo sobre las probabilidades, y nosotros seremos honestos con usted.'} },
    { id:'w6',
      label:{en:'I was discriminated against, harassed, or punished for speaking up', es:'Sufrí discriminación, acoso o represalias por quejarme'},
      forum:'CRD · EEOC',
      title:{en:'A civil-rights complaint starts with the CRD.', es:'Una queja de derechos civiles empieza en el CRD.'},
      path:{en:['File an intake with the CRD — dual-filed with the EEOC where it applies','The complaint is drafted, signed, and served; the employer responds','Investigation, with free mediation offered along the way — many cases settle here','If it doesn\u2019t resolve: right-to-sue notice, then 1 year to file in court'],
            es:['Presente la queja inicial ante el CRD, registrada también ante la EEOC cuando aplica','La queja se redacta, se firma y se notifica; el empleador responde','Investigación, con mediación gratuita en el camino: muchos casos se arreglan aquí','Si no se resuelve: aviso de derecho a demandar, y 1 año para ir al tribunal']},
      clock:{en:'3 years from the last discriminatory act to file with the CRD (Gov. Code §12960). After a right-to-sue notice: 1 year to file suit.', es:'3 años desde el último acto discriminatorio para presentar ante el CRD (Código de Gobierno, §12960). Después del aviso de derecho a demandar: 1 año para demandar.'},
      win:{years:3},
  ...
  employer: [
    { id:'e1',
      label:{en:'A former employee filed for UI and I got a hearing notice', es:'Un exempleado pidió desempleo y me llegó un aviso de audiencia'},
      forum:'EDD · CUIAB',
    ...
    { id:'e2',
      ...
      clock:{en:'The CRD sets your response date in its letter. It\u2019s short — extensions are requested, not assumed.', es:'El CRD fija su fecha de respuesta en la carta. Es corta: las prórrogas se piden, no se dan por hechas.'},
      win:null,
    ...
    { id:'e3',
      label:{en:'EDD is auditing my payroll or sent a Notice of Assessment', es:'El EDD está auditando mi nómina o me mandó un Aviso de Liquidación'},
      forum:'EDD · CUIAB',
      ...
      path:{en:['Petition for reassessment within 30 days — this preserves everything','Assemble the working relationship: contracts, invoices, who controls the how','The petition goes before a CUIAB judge in the tax setting','Decision, with a further appeal to the Board available'],
      ...
      clock:{en:'30 days from the Notice of Assessment to petition for reassessment (Unemp. Ins. Code §1222).', es:'30 días desde el Aviso de Liquidación para pedir la reevaluación (Código de Seguro de Desempleo, §1222).'},
      win:{days:30},
...
var TSTR = {
  en:{ pathH:'Your path', clockH:'Your clock', gatherH:'Gather now', dateL:'Date printed on your notice', calc:'Compute my date',
       ...
       from:'Counted from the date on the notice. Weekends roll forward; state holidays also roll, but check them yourself.',
       cta:'Get a free case review', ctaNote:'Send the notice with the form — we reply within a business day.' },
  es:{ pathH:'Su camino', clockH:'Su reloj', gatherH:'Junte desde ahora', dateL:'Fecha impresa en su aviso', calc:'Calcular mi fecha',
       ...
       from:'Contado desde la fecha del aviso. Los fines de semana se recorren; los feriados estatales también, pero revíselos usted.',
       cta:'Revisión gratuita de su caso', ctaNote:'Mande el aviso con el formulario: respondemos en un día hábil.' },
};
...
function esc(x){ return x; } /* all strings are first-party */
...
function renderTriage(){
  ...
  html += '<h3>' + esc(s.title[LANG]) + '</h3>';
  html += '<div class="tri-h">' + t.pathH + '</div><ol class="tri-path">';
  s.path[LANG].forEach(function(p){ html += '<li>' + esc(p) + '</li>'; });
  html += '</ol>';
  html += '<div class="tri-h">' + t.clockH + '</div><p class="tri-clock">' + esc(s.clock[LANG]) + '</p>';
  if (s.win){
    html += '<div class="tri-date"><div><label class="field" for="triD">' + t.dateL + '</label>' +
            ...
            '<button class="btn" type="button" onclick="triCalc()">' + t.calc + '</button></div>' +
            '<div class="dl-out" id="dlOut"><div class="dl-label small" style="text-transform:uppercase;letter-spacing:.1em;font-weight:700">' + t.fileBy + '</div>' +
            '<div class="dl-date" id="dlDate"></div><div class="dl-pill" id="dlPill"></div><div class="dl-note" id="dlNote"></div></div>';
  }
  html += '<div class="tri-h">' + t.gatherH + '</div><ul class="tri-gather">';
  s.gather[LANG].forEach(function(g){ html += '<li>' + esc(g) + '</li>'; });
  html += '</ul>';
  html += '<p class="tri-note">' + esc(s.note[LANG]) + '</p>';
  html += '<div class="tri-cta"><a class="btn il" href="' + withLang('contact.html', LANG) + '">' + t.cta + '</a><p class="small">' + t.ctaNote + '</p></div>';
...
function triCalc(){
  ...
  if (diff < 0){ out.classList.add('past'); pill.textContent = t.past(-diff); note.textContent = t.pastNote; }
  else if (diff === 0){ out.classList.add('warn'); pill.textContent = t.today; note.textContent = t.from; }
  else { out.classList.add(diff <= 14 ? 'warn' : 'ok'); pill.textContent = t.left(diff); note.textContent = t.from; }
